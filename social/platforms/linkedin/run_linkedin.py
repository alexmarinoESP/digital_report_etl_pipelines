#!/usr/bin/env python3
"""
LinkedIn Ads ETL Pipeline Runner.

Standalone entry point for Azure Container App deployment.
This script orchestrates the complete LinkedIn Ads data extraction pipeline:
...
def setup_logging(log_level: str = "INFO") -> None:
    ...
    if os.getenv("LOG_TO_FILE", "false").lower() == "true":
        log_file = Path("logs") / f"linkedin_ads_{datetime.now():%Y%m%d}.log"
...
def load_configuration() -> Dict[str, Any]:
    """
    Load LinkedIn Ads platform configuration from YAML.
    ...
    try:
        ...
        # Extract table names
        table_names = [k for k in config.keys() if k != "platform" and k.startswith("linkedin_ads_")]
...
def setup_azure_table_sink() -> Any:
    ...
    try:
        ...
        table_name = os.getenv("AZURE_TABLE_NAME", "linkedinads")
...
def main() -> int:
    ...
    try:
        ...
        logger.info("\n[2/4] Setting up token provider...")
        token_provider = setup_token_provider()

        # Step 3: Setup data sink
        logger.info("\n[3/4] Setting up data sink...")
