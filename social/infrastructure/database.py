...
class VerticaDataSink:
    ...
    def load(
        ...
        try:
            ...
            # DEBUG: Log DataFrame columns before processing
            logger.debug(f"DataFrame columns BEFORE alignment: {list(df.columns)}")
            logger.debug(f"'load_date' in DataFrame: {'load_date' in df.columns}")
