...
class SocialPipeline:
    ...
    def _run_platform(self, platform_name: str, tables: Optional[List[str]] = None) -> Dict[str, int]:
        ...
        try:
            ...
            if hasattr(adapter, 'extract_all_tables'):
                ...
                # Google style: run_all_tables(tables=...) with built-in filtering
                dataframes = adapter.run_all_tables(load_to_sink=False, tables=tables_to_extract)
            else:
            ...
            for table_name, df in dataframes.items():
                ...
                if self.config.dry_run:
                    ...
                    # Determine load mode from table config
                    table_config = adapter.config.get(table_name, {})
                    load_mode = self._determine_load_mode(table_config, table_name)

                    logger.info(f"Loading {row_count} rows to {table_name} (mode={load_mode})...")

                    # Prepare load parameters
                    load_params = {"df": df, "table_name": table_name, "mode": load_mode}
                    if load_mode == "increment":
                        inc_cfg = table_config.get("increment", {})
                        load_params["dedupe_columns"] = inc_cfg.get("pk_columns")
                        load_params["increment_columns"] = inc_cfg.get("increment_columns")
                    elif load_mode == "upsert":
                        # Support both new (upsert) and legacy (merge/update) config formats
                        ups_cfg = table_config.get("upsert") or table_config.get("merge") or table_config.get("update") or {}
    ...
    def _determine_load_mode(self, table_config: Dict[str, Any], table_name: str) -> str:
        ...
        Returns:
            Load mode: 'replace', 'increment', 'upsert', or 'append'
        """
        if table_config.get("increment"):
            logger.debug(f"Using INCREMENT mode for {table_name}")
            return "increment"

        elif table_config.get("upsert"):
            logger.debug(f"Using UPSERT mode for {table_name}")
            return "upsert"

        elif table_config.get("replace") or table_config.get("truncate", False):
            logger.warning(f"✓ Using REPLACE mode for {table_name} (truncate={table_config.get('truncate')}, replace={table_config.get('replace')})")
            return "replace"

        elif table_config.get("update") or table_config.get("merge"):
            # Legacy configs: map to upsert
            logger.debug(f"Using UPSERT mode for {table_name} (legacy config)")
            return "upsert"

        else:
            # Default: APPEND (insert only new rows, skip duplicates)
            logger.debug(f"Using APPEND mode for {table_name} (default)")
            return "append"
