...
-- Handles type mismatches and corrupt data - Vertica compatible
-- ============================================================================
...
-- Copy fb_ads_ad_set (filter out invalid end_time values)
...
SELECT
    ...
    CASE
        ...
        WHEN end_time::VARCHAR = '' THEN NULL
        ELSE end_time
    END,
    destination_type,
    campaign_id,
...
WHERE end_time IS NULL OR end_time::VARCHAR NOT LIKE '%invalid%';
...
-- Copy fb_ads_insight_actions (filter out invalid value)
...
SELECT
    ...
    CASE
        ...
        WHEN value::VARCHAR = '' THEN NULL
        ELSE value
    END,
    COALESCE(last_updated_date::DATE, row_loaded_date::DATE, CURRENT_DATE)
FROM GoogleAnalytics.fb_ads_insight_actions
WHERE value IS NULL OR value::VARCHAR NOT LIKE '%invalid%';
